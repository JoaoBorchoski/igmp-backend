...
class CidadeRepository implements ICidadeRepository {
    ...
    async getByName(nomeCidade: string, estadoId: string): Promise<HttpResponse> {
        try {
            const cidade = await this.repository
                .createQueryBuilder("cid")
                .select([
                ...
                .leftJoin("cid.estadoId", "a")
                .where("unaccent(cid.nomeCidade) = unaccent(:nomeCidade)", {
                    nomeCidade: nomeCidade,
                })
                .andWhere("cid.estadoId = :estadoId", { estadoId })
                .getRawOne()

            if (typeof cidade === "undefined") {
                const cidade2 = await this.repository
                    .createQueryBuilder("cid")
                    .select([
                        'cid.id as "id"',
                        'cid.estadoId as "estadoId"',
                    ...
                    .leftJoin("cid.estadoId", "a")
                    .where("cid.id = '55bfa279-581a-45bf-bc2b-1ea3ff408d21'")
                    .getRawOne()
