	...
	async execute({
		...
		try {
			...
			if (alreadyHasPacote.statusCode === 200) {
				return conflictError('Pedido já tem pacote')
			}
			...
			const itemsApagados = itemsPedidos.data.filter((item) => !pedidoItems.some((pedidoItem) => pedidoItem.id === item.produto))
			...
			for await (const item of pedidoItems) {
				...
				await this.pedidoItemRepository.updateWithQueryRunner({ id: pedidoItem.data.id, quantidade: item.quantidade }, queryRunner.manager)
			}
