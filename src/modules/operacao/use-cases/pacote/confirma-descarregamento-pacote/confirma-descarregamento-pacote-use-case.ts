...
@injectable()
class ConfirmaDescarregamentoPacoteUseCase {
	constructor(
	...
	async execute({ id, espelhoCargaId }: IRequest): Promise<HttpResponse> {
		const pacote = await this.pacoteRepository.updatePacoteItemDescarregadoStatus(id, espelhoCargaId)

		return pacote
	}
}

export { ConfirmaDescarregamentoPacoteUseCase }
