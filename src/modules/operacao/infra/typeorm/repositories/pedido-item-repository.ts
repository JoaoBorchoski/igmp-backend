...
class PedidoItemRepository implements IPedidoItemRepository {
	...
	async createWithQueryRunner({ pedidoId, produto, quantidade, corEtiqueta, kit }: IPedidoItemDTO, transactionManager: EntityManager): Promise<HttpResponse> {
		try {
	...
	async list(search: string, page: number, rowsPerPage: number, order: string, filter: string): Promise<HttpResponse> {
		...
		try {
			let query = this.repository
				...
				.select(['ped.id as "id"', 'a.id as "pedidoId"', 'a.sequencial as "pedidoSequencial"', 'ped.quantidade as "quantidade"', 'ped.corEtiqueta as "corEtiqueta"'])
	...
	async selectByPedidoId(filter: string, pedidoId: string): Promise<HttpResponse> {
		try {
			...
			if (filter) {
				...
				query = query.orWhere('p.descricao ilike :filter', { filter: `${filter}%` })
			}

			const pedidosItems = await query.addOrderBy('ped.pedidoId').getRawMany()

			const produtosOpcinais = await this.repository.query(
				`
					SELECT 
                        id as "value",
                        nome || ' - ' || descricao as "label"
                    FROM 
                        produtos 
                    WHERE 
                        opcional = true OR tipo = 2
				`
			)

			pedidosItems.push(...produtosOpcinais)
	...
	async getProduto(id: string): Promise<HttpResponse> {
		try {
			...
			if (typeof pedidoItem === 'undefined' && typeof produtosOpcinais === 'undefined') {
				return noContent()
			}

			return ok(pedidoItem ?? produtosOpcinais[0])
		} catch (err) {
	...
	async updateWithQueryRunner({ id, pedidoId, produto, quantidade, corEtiqueta }: IPedidoItemDTO, transactionManager: EntityManager): Promise<HttpResponse> {
