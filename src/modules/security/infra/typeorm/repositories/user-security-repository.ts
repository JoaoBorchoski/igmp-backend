...
class UserSecurityRepository implements IUserSecurityRepository {
	...
	async list(
		...
		const referenceArray = ["userGroupName", "name", "login", "description"]
	...
	// select
	async select(filter: string, isAdmin: boolean, isSuperUser: boolean, userGroupId: string): Promise<HttpResponse> {
		try {
			let users = []
			...
			if (!isSuperUser) {
				...
				users = await this.repository
					.createQueryBuilder("use")
					.select(['use.id as "value"', "CONCAT(us1.name, ' - ', use.name) as \"label\""])
	...
	async getUserMenu(user: IUserDTO): Promise<HttpResponse> {
      ...
      `)
			...
			convertedMenus.map((convertedMenu) => {
				convertedMenu.subMenuOptions = convertedMenu.subMenuOptions.filter(
					(subMenu) =>
						subMenu.permitAll === true ||
